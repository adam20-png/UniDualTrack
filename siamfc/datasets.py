...
class Pair(Dataset):
    ...
    def __init__(self, seqs, transforms=None,
                 pairs_per_seq=1, use_advanced_aug=True):
        super(Pair, self).__init__()
        self.seqs = seqs
        # Si `transforms` est une instance de SiamFCTransforms, on peut la
        # ré-instancier pour activer/désactiver les augmentations avancées.
        if transforms is None:
            try:
                from siamfc.transforms import SiamFCTransforms
                transforms = SiamFCTransforms(use_advanced_aug=use_advanced_aug)
            except Exception:
                transforms = transforms
        else:
            try:
                from siamfc.transforms import SiamFCTransforms
                if isinstance(transforms, SiamFCTransforms):
                    transforms = SiamFCTransforms(
                        exemplar_sz=transforms.exemplar_sz,
                        instance_sz=transforms.instance_sz,
                        context=transforms.context,
                        use_advanced_aug=use_advanced_aug,
                    )
            except Exception:
                # On garde le comportement d'origine si on ne peut pas
                # interpréter / réinstancier les transforms.
                pass
