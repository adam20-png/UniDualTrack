...
class VGG166(nn.Module):
    def __init__(self):
        ...
        # === MODIFIED SECTION ===
        # Fix 1: Learned strided convolution replacing blunt max-pool on branch1.
        # stride=2, padding=0 matches the crop-style border handling of this backbone.
        self.branch1_adapt = nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=0, bias=False),
            _BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )
        # Fix 2: Lightweight depthwise-separable decorrelation head on branch1.
        # Gives branch1 its own exclusive parameters so it can diverge from the
        # shared VGG path and approximate the representational diversity that
        # the removed AlexNet naturally provided.
        # Extra params: ~68K — negligible vs. the backbone.
        self.branch1_decorr = nn.Sequential(
            nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1,
                      groups=256, bias=False),      # depthwise
            _BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, kernel_size=1, stride=1, bias=False),  # pointwise
            _BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )
    ...
    # === MODIFIED SECTION ===
    # Forward now returns TWO feature branches from a SINGLE VGG backbone pass:
    #   branch1 = conv3_3 output  (mid-level spatial features, 256ch)
    #             → preserves spatial resolution for precise localization
    #   branch2 = conv5_1 output  (deep semantic features, 256ch via 1×1 proj)
    #             → captures semantic context for appearance-change robustness
    # The backbone runs ONCE per image. No AlexNet is used.
    def forward(self, x):
        x = self.conv1_1(x)
        x = self.bn1_1(x)
        x = self.re1(x)
        ...
        # === MODIFIED SECTION ===
        # Branch 1: conv3_3 features processed through two dedicated sub-modules.
        #
        # Fix 1 — branch1_adapt: learned strided conv (stride=2, 3×3, no padding)
        #   replaces the previous blunt max-pool. The network now LEARNS which
        #   spatial structure to keep, preserving localisation detail instead of
        #   blindly discarding half the feature map.
        #
        # Fix 2 — branch1_decorr: depthwise-separable block applied AFTER adapt.
        #   Both branches share VGG weights up to this point (correlated by design).
        #   This block gives branch1 its own exclusive trainable parameters so it
        #   can specialise away from the shared path — recovering the representational
        #   diversity that the removed AlexNet provided for free via separate weights.
        branch1 = self.branch1_adapt(x)     # Fix 1: learned spatial downsampling
        branch1 = self.branch1_decorr(branch1)  # Fix 2: branch-exclusive decorrelation
        # === END MODIFIED SECTION ===
        ...
        # === MODIFIED SECTION ===
        # Branch 2: conv5_1 output — deep semantic features (512→256ch via 1×1 conv)
        # Projected to 256ch to match the SiamFC head's expected channel dimension.
        branch2 = self.conv5_1(x)           # 256ch deep semantic features
        # === END MODIFIED SECTION ===

        # === MODIFIED SECTION ===
        # Return BOTH branches from this single forward pass.
        # branch1 = mid-level (conv3_3 spatial),  branch2 = deep (conv5_1 semantic)
